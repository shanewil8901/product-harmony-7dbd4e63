...
export function EmployeeViewPage() {
  ...
  /** Supervisors may view people data but never salary, bank or payroll. */
  const { canViewConfidentialHr } = usePermissions();
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      setData(await employeesService.overview(id));
    } catch (e) {
      setError((e as ApiError).userMessage ?? 'Could not load this employee');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) return <div className="card p-8 text-sm text-brown-500">Loading employee…</div>;

  if (error || !data)
    return (
      <div className="card p-8 text-center">
        <h1 className="font-serif text-2xl text-ink">Employee</h1>
        <p className="mt-2 text-sm text-brown-600">{error ?? 'Employee not found.'}</p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <h1 className="font-serif text-2xl text-ink">Employee profile</h1>
        <Link to="/employees" className="btn-ghost text-sm">
          ← Back to employees
        </Link>
      </div>
      ...
      {e.iqama_days_left !== null && e.iqama_days_left <= 60 && (
        ...
        >
          {e.iqama_days_left < 0
            ? `Iqama expired on ${day(e.iqama_expiry)}. Renewal is overdue.`
            : `Iqama expires in ${e.iqama_days_left} day(s) on ${day(e.iqama_expiry)}.`}
        </div>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <section
          className={`card p-5 space-y-4 ${canViewConfidentialHr ? 'lg:col-span-2' : 'lg:col-span-3'}`}
        >
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="card p-5">
          <h2 className="font-serif text-lg text-ink mb-2">Documents</h2>
          <ul className="space-y-2">
            ...
            {documents.length === 0 && (
              <li className="text-sm text-brown-500">No documents on file.</li>
            )}
          </ul>
        </section>
      </div>
