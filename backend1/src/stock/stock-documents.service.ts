...
export class StockDocumentsService {
  ...
  async createForStock(
    ...
    return this.dataSource.transaction(async (mgr) => {
      ...
      // Skipping is forward-only: an admin/manager may jump ahead in the
      // pipeline, but no one may move a stock row back to an earlier stage.
      if (opts.skipTransitionCheck && !isForwardStage(docType, stock.status)) {
        throw new BadRequestException(
          `The lifecycle cannot be moved backwards — this stock is already at "${stock.status}".`,
        );
      }
      ...
      // Immutable stage audit entry — records the actor, their role and the
      // exact reference data submitted for this stage.
