...
export class SalesService {
  ...
  /**
   * Commission is a share of the PROFIT (selling value minus snapshotted cost),
   * never of the sales value, and it is only earned once the company actually
   * receives the cash — so it accrues in proportion to the amount received.
   */
  private commissionSummary(o: SalesOrder) {
    const items = o.items ?? [];
    const revenue = items.reduce((s, i) => s + Number(i.line_total), 0);
    const cost = items.reduce((s, i) => s + Number(i.unit_cost_snapshot) * Number(i.qty), 0);
    const profit = revenue - cost;
    const percent = Number(o.commission_percent ?? 10);
    const commission = Math.max((profit * percent) / 100, 0);

    const total = Number(o.total_amount);
    const paid = Number(o.amount_paid);
    const ratio = total > 0 ? Math.min(Math.max(paid / total, 0), 1) : 0;
    const status: 'pending' | 'partial' | 'earned' | 'cancelled' =
      o.status === 'cancelled' ? 'cancelled' : ratio >= 1 ? 'earned' : ratio > 0 ? 'partial' : 'pending';

    return {
      profit_amount: money(profit),
      commission_percent: percent.toFixed(2),
      commission_amount: money(commission),
      commission_earned_amount: money(status === 'cancelled' ? 0 : commission * ratio),
    ...
    const id = await this.dataSource.transaction(async (mgr) => {
      const order = await mgr.save(
        mgr.create(SalesOrder, {
          ...
          salesperson_id: dto.salesperson_id ?? actor,
          commission_percent: (dto.commission_percent ?? 10).toFixed(2),
          created_by: actor,
          updated_by: actor,
  ...
  async update(id: string, dto: UpdateSalesOrderDto, actor: string) {
    ...
    if (dto.salesperson_id !== undefined) order.salesperson_id = dto.salesperson_id ?? null;
    if (dto.commission_percent !== undefined)
      order.commission_percent = Number(dto.commission_percent).toFixed(2);
