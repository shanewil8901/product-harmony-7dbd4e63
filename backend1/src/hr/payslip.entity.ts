...
export class Payslip {
  ...
  /** Working days configured for the month — the daily-rate divisor. */
  @Column({ name: 'working_days', type: 'int', default: 0 })
  working_days!: number;

  /** Approved leave inside the entitlement (fully paid). */
  @Column({ name: 'paid_leave_days', type: 'decimal', precision: 6, scale: 2, default: '0.00' })
  paid_leave_days!: string;

  /** Approved leave beyond the entitlement — deducted at the daily rate. */
  @Column({ name: 'unpaid_leave_days', type: 'decimal', precision: 6, scale: 2, default: '0.00' })
  unpaid_leave_days!: string;

  /** True when the scheduler issued this payslip rather than a person. */
  ...
  /** Reason captured when the payslip is cancelled. */
  @Column({ name: 'cancel_reason', type: 'text', nullable: true })
  cancel_reason!: string | null;

  /** Soft deletion marker — rows are never physically removed. */
  @Index()
