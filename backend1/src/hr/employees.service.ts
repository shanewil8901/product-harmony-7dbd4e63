...
import { Between, DataSource, IsNull, Repository, Not} from 'typeorm';
...
import { BOOTSTRAP_ADMIN_EMAIL } from '../users/bootstrap';
...
export class EmployeesService {
  ...
  async create(dto: CreateEmployeeDto, actor: string) {
    ...
    return this.dataSource.transaction(async (mgr) => {
      ...
      // First real admin created → retire the temporary bootstrap account.
      if (dto.role === 'admin' && dto.email !== BOOTSTRAP_ADMIN_EMAIL) {
  ...
  listDocuments(employeeId: string) {
    // The profile photo is managed separately and is not a filing-cabinet document.
    return this.docRepo.find({
      where: { employee_id: employeeId, doc_type: Not('photo' as const) },
  ...
  /** Optional passport-size profile photo — one per employee, replaced on re-upload. */
  async findPhoto(employeeId: string) {
    return this.docRepo.findOne({
      where: { employee_id: employeeId, doc_type: 'photo' },
      order: { uploaded_at: 'DESC' },
    });
  }

  async findPhotoByUser(userId: string) {
    const profile = await this.repo.findOne({ where: { user_id: userId } });
    if (!profile) throw new NotFoundException('No employee profile is linked to your account');
    return this.findPhoto(profile.id);
  }

  async setPhoto(employeeId: string, file: Express.Multer.File, actor: string) {
    await this.findOne(employeeId);
    const existing = await this.docRepo.find({
      where: { employee_id: employeeId, doc_type: 'photo' },
    });
    if (existing.length) await this.docRepo.remove(existing);
    return this.docRepo.save(
      this.docRepo.create({
        employee_id: employeeId,
        doc_type: 'photo',
  ...
  async removePhoto(employeeId: string) {
    const rows = await this.docRepo.find({ where: { employee_id: employeeId, doc_type: 'photo' } });
    if (rows.length) await this.docRepo.remove(rows);
  ...
  async selfOverview(userId: string) {
    ...
    return this.overviewFor(profile);
  }

  /** Same read-only dashboard for any employee — used by the View modal. */
  async employeeOverview(id: string) {
    const profile = await this.repo.findOne({
      where: { id },
      relations: ['user', 'user.role', 'department', 'salary_currency'],
    });
  ...
  private async overviewFor(profile: EmployeeProfile) {
    ...
    const [monthRows, yearRows, payslips, documents] = await Promise.all([
      ...
      this.payslipRepo.find({
        where: { employee_id: profile.id, deleted_at: IsNull() },
        order: { period: 'DESC' },
        take: 12,
      }),
      this.docRepo.find({
        where: { employee_id: profile.id, doc_type: Not('photo' as const) },
        order: { uploaded_at: 'DESC' },
      }),
    ]);
