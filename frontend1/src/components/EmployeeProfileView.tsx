...
/**
 * The shared employee profile layout. Used by "My profile" and by the
 * read-only View modal on the Employees screen so both stay identical.
 */
    ...
    <div className="space-y-6">
      <header className="card overflow-hidden">
        <div className="flex flex-col gap-5 bg-gradient-to-r from-paper-warm to-paper p-5 sm:flex-row">
          <div className="w-28 shrink-0 overflow-hidden rounded-xl border-2 border-gold-300 bg-paper-warm shadow-card">
            <img
      ...
      {e.iqama_days_left !== null && e.iqama_days_left <= 60 && (
        ...
        >
          {e.iqama_days_left < 0
            ? `${self ? 'Your' : 'This'} Iqama expired on ${day(e.iqama_expiry)}.`
            : `${self ? 'Your' : 'This'} Iqama expires in ${e.iqama_days_left} day(s) on ${day(e.iqama_expiry)}.`}
        </div>
      )}
