...
import { isKsaMobileLocal, MOBILE_HELP, toLocalMobile } from '../lib/validators';
...
export function CustomerModal({ customer, canEdit, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    if (customer) {
      setForm({
        ...
        phone: toLocalMobile(customer.phone),
  ...
  const validate = (): boolean => {
    ...
    if (!isKsaMobileLocal(form.phone)) e.phone = MOBILE_HELP;
      ...
      >
        ...
        {tab === 'info' && (
          <div className="space-y-4">
            ...
            <Pair>
              <Field label="Mobile (KSA) *" error={errors.phone}>
                <input
                  className="input font-mono"
                  value={form.phone}
                  inputMode="numeric"
