...
import { documentPrintUrl } from "../services/stockDocuments.service";
...
export function StockPage() {
  ...
  const uomMap = useMemo(() => Object.fromEntries(uoms.map((u) => [u.id, u.code])), [uoms]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[1050px]">
            ...
            <tbody className="divide-y divide-brown-50">
              {loading && (
                <tr>
                  <td colSpan={10} className="py-8 text-center text-brown-500 whitespace-nowrap">
                    Loading…
                  </td>
                </tr>
              )}
              {!loading && items.length === 0 && (
                <tr>
                  <td colSpan={10} className="py-12 text-center text-brown-500 whitespace-nowrap">
                    No stock orders yet.
                  </td>
                </tr>
              )}
              {!loading &&
                items.map((s) => {
                  ...
                  const uom = s.qty_uom_code ?? (s.qty_uom_id ? uomMap[s.qty_uom_id] : "");
                  const buyCur =
                    s.buying_currency_code ??
                    (s.buying_currency_id_snapshot ? curMap[s.buying_currency_id_snapshot] : "");
                  const sellCur =
                    s.selling_currency_code ??
                    (s.selling_currency_id_snapshot ? curMap[s.selling_currency_id_snapshot] : "");
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-paper-soft">
                      <Td className="whitespace-nowrap min-w-[200px]">
                        <div className="font-mono text-ink font-medium">{productCode}</div>
                      ...
                      <Td className="font-mono text-xs whitespace-nowrap">{s.batch_no ?? "—"}</Td>
                      ...
                      <Td align="right" className="text-brown-500 whitespace-nowrap font-mono">
                        {formatMoney(s.buying_price_snapshot, buyCur)}
                      </Td>
                      <Td
                        align="right"
                        className="text-ink font-semibold whitespace-nowrap font-mono"
                      >
                        {formatMoney(totalBuying, buyCur)}
                      </Td>
                      <Td
                        align="right"
                        className="text-forest-500 font-medium whitespace-nowrap font-mono"
                      >
                            ...
                            <button
                              ...
                              onClick={async () => {
                                const ok = await confirm({
                                  ...
                                  message:
                                    "Delete this stock order? Its documents and history stay in the audit trail.",
                                  confirmLabel: "Delete",
                                  tone: "danger",
                                });
                                if (!ok) return;
      ...
      {modalOpen && (
        <StockModal
          ...
          onSubmit={async (input) => {
            ...
            setModalOpen(false);
            // Immediately preview the auto-generated first document in-app.
            setPreviewDoc({ id: po_document.id, label: po_document.doc_number });
            refresh();
          }}
        />
      )}
      ...
      {historyFor && <StockHistoryPanel stock={historyFor} onClose={() => setHistoryFor(null)} />}
      {dialog}
    </div>
  );
}
...
function StockModal({ products, onClose, onSubmit }: ModalProps) {
  ...
  const estimatedValue =
    selectedProduct && qty ? (Number(qty) * Number(selectedProduct.buyingPrice)).toFixed(2) : null;
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {/* Locked to the product's base UOM — stock is always counted in it. */}
            <input
              className="input bg-paper-warm text-brown-600 cursor-not-allowed"
              readOnly
              disabled
              value={
                uomOptions.find((o) => o.value === qtyUom)?.label ??
                (productId ? "—" : "Select a product first")
              }
            />
            <p className="text-xs text-brown-500 mt-1">
              Taken from the product's base unit and cannot be changed.
            </p>
          </div>
        ...
        <div className="rounded-lg border border-brown-100 bg-paper-warm px-3 py-2 text-sm">
          ...
          <span className="text-ink font-semibold">{formatMoney(estimatedValue, buyCurrency)}</span>
        </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // An expiry can never precede the manufacture date.
                if (exp && e.target.value && exp < e.target.value) setExp(e.target.value);
              }}
            />
          </div>
          <div>
...
function Th({ children, align = "left" }: { children: React.ReactNode; align?: "left" | "right" }) {
  return (
    <th
      className={`px-4 py-3 text-xs whitespace-nowrap font-semibold uppercase tracking-wider ${align === "right" ? "text-right" : ""}`}
    >
      {children}
    </th>
  );
