...
export function VendorsPage() {
  ...
  const remove = async (v: Vendor) => {
    const ok = await confirm({
      title: "Delete vendor",
      message: `Delete vendor "${v.legal_name}"? This removes all uploaded documents.`,
      confirmLabel: "Delete",
      tone: "danger",
    });
    if (!ok) return;
              ...
              <tbody>
                {rows.map((v) => (
                  <tr key={v.id} className="border-t border-brown-100">
                    <td className="px-4 py-3 font-mono text-ink whitespace-nowrap">{v.code}</td>
