...
export function EmployeesPage() {
  ...
  const [viewData, setViewData] = useState<EmployeeSelfOverview | null>(null);
  const [viewPhoto, setViewPhoto] = useState<string | null>(null);
  const [viewError, setViewError] = useState<string | null>(null);

  const openView = async (e: Employee) => {
    setViewing(e);
    setViewData(null);
    setViewPhoto(null);
    setViewError(null);
    try {
      setViewData(await employeesService.overview(e.id));
    } catch (err) {
      setViewError((err as ApiError).userMessage ?? 'Could not load this employee');
    }
    try {
      setViewPhoto(await employeesService.photoUrl(e.id));
    } catch {
      setViewPhoto(null);
    }
  };
  ...
  const closeView = () => {
    if (viewPhoto) URL.revokeObjectURL(viewPhoto);
    setViewPhoto(null);
    setViewData(null);
    setViewing(null);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {viewing && (
        <Modal title={viewing.full_name} subtitle={viewing.employee_code} onClose={closeView} wide>
          {viewError ? (
            <p className="text-sm text-brown-600">{viewError}</p>
          ) : viewData ? (
            <EmployeeProfileView data={viewData} photo={viewPhoto} />
          ) : (
            <p className="text-sm text-brown-500">Loading employee profile…</p>
          )}
        </Modal>
      )}
