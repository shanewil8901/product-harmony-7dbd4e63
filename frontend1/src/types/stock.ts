...
/**
 * Linear pipeline order + how far each status sits along it. Used to keep
 * lifecycle skips forward-only (mirrors the backend `STAGE_SEQUENCE`).
 */
...
/** True when the stage is ahead of (never behind) the current status. */
